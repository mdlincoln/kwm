...
test_that("Invalid inputs are rejected", {
  expect_error(kwm(include_list = integer(1)))
  expect_error(kwm(exclude_list = integer(1)))
...
test_that("Invalid prediction outputs are rejected", {
  bad_kwm_model <- kwm(include_list = c("a", "y"), exclude_list = "r", varname = "foo")
...
test_that("Matches inclusions correctly", {
  kwm_model <- kwm(include_list = c("a", "y"), exclude_list = "r", varname = "month")
